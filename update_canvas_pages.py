...
def find_mapping_entry(file_key, page_mapping):
    """Find a mapping entry for a file, handling Canvas URL suffixes.

    Canvas may add suffixes like -2, -3 to URLs when pages with the same title exist.
    This function tries to match the file_key to mapping entries even if the mapping
    has a different suffix.

    For example:
    - file_key: "module1/hva-skal-du-laere.html"
    - mapping has: "hva-skal-du-laere.html" with url "hva-skal-du-laere-3"
    - Should still match!

    Args:
        file_key: Relative path to HTML file (e.g., "module1/hva-skal-du-laere.html")
        page_mapping: Dictionary mapping filenames to Canvas page info

    Returns:
        Tuple of (matched_key, page_info) or (None, None) if no match found
    """
    import re

    # First try exact match
    if file_key in page_mapping:
        return file_key, page_mapping[file_key]

    # Extract the base filename without module path for fuzzy matching
    # e.g., "module1/hva-skal-du-laere.html" -> "hva-skal-du-laere.html"
    file_parts = file_key.split('/')
    filename_only = file_parts[-1] if len(file_parts) > 0 else file_key

    # Try matching just the filename (backward compatibility)
    if filename_only in page_mapping:
        return filename_only, page_mapping[filename_only]

    # Try fuzzy matching: look for entries where the base name matches
    # Remove .html extension for comparison
    base_name = filename_only.replace('.html', '')

    for mapping_key, page_info in page_mapping.items():
        # Extract filename from mapping key (may include module path)
        mapping_parts = mapping_key.split('/')
        mapping_filename = mapping_parts[-1] if len(mapping_parts) > 0 else mapping_key
        mapping_base = mapping_filename.replace('.html', '')

        # Check if they match when ignoring Canvas suffixes (-2, -3, etc.)
        # Remove any trailing -N suffix from mapping base
        mapping_base_clean = re.sub(r'-\d+$', '', mapping_base)
        file_base_clean = re.sub(r'-\d+$', '', base_name)

        if mapping_base_clean == file_base_clean:
            # Found a match!
            return mapping_key, page_info
...
def process_html_files(token, html_files, html_dir, page_mapping, args, page_id_override=None):
    ...
    for html_file in html_files:
        ...
        # Try to find page info from mapping file using fuzzy matching
        # This handles Canvas URL suffixes like -2, -3
        matched_key, page_info = find_mapping_entry(file_key, page_mapping)

        if matched_key and matched_key != file_key:
            print(f"Matched mapping: {matched_key} (fuzzy match)")
        ...
        if should_create_new:
            ...
            if page_updated and actual_page_url and not args.dry_run:
                if page_info and matched_key:
                    # Update existing mapping with actual URL from Canvas
                    # Use matched_key (the actual key in mapping) not file_key
                    page_mapping[matched_key]['url'] = actual_page_url
                    if title:
                        page_mapping[matched_key]['title'] = title
