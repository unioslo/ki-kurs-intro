
Hva er kunstig intelligens (KI)?
================================

Kunstig intelligens (KI) er teknologi som gjør det mulig for datamaskiner å utføre oppgaver som normalt krever menneskelig intelligens. 
Dette kan være alt fra å gjenkjenne ansikter i bilder til å forstå og svare på spørsmål.

KI er ikke en ny teknologi - den har utviklet seg over flere tiår: allerede i 1956 ble det etablert et forskningsfelt innen KI.
Men siden ChatGTP ble sluppet i 2022 har vi sett en eksplosjonsartet utvikling av KI. ChatGPT var milevis fra de "dumme chatbotene" som kun klarte å svare på et smalt spekter av spørsmål. 
Plutselig hadde alle tilgang til en tjeneste som kunne *generere* side på side med tekst av overraskende god kvalitet.  

Denne utviklingen har vært mulig takket være tre gjennombrudd: **enorme mengder treningsdata fra internett**, kraftige **GPU-er** som kan behandle massive datamengder parallelt, og nye **revolusjonerende teknikker** (*transformers* mer om dette senere) som gir ekstremt gode resultater.

.. uio-note:: Oppsummert

   * Kunstig intelligens har utviklet seg fra regelstyrt tradisjonell KI til moderne KI som lærer av enorme datamengder. 
   * Den eksplosjonsartede utviklingen siden 2022 ble muliggjort av transformer-teknologi og dype nevrale nettverk. 
   * Moderne KI kan nå generere tekst, gjenkjenne mønstre og tilpasse seg nye situasjoner på måter tradisjonell KI aldri kunne.
