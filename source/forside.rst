Velkommen til kurset:  Introduksjon til kunstig intelligens
============================================================================

Kunstig intelligens er en avansert teknologi som er lett å bruke, men vanskelig å bruke riktig. 
Dette er en introduksjon til hva kunstig intelligens er, og hvordan du kan bruke den på en god og ansvarlig måte.
Kurset skal bidra til at alle ansatte får grunnleggende opplæring i kunstig intelligens, og dermed trygt kan utforske hvordan KI kan brukes i egne arbeidsoppgaver.

Kurset er utviklet for administrativt ansatte, men temaene er grunnleggende og relevante for alle ansatte som ønsker å lære om hvordan bruke kunstig intelligens.

.. figure:: images/0.5-greeting.png
   :width: 60%
   :alt: Illustrasjon til forsiden av grunnkurs i generativ KI. Mennesket (Pelle) og roboten (Plimbo) hilser på hverandre.

   Illustrasjon: Tina Morønning Ruud

Hvorfor er dette kurset viktig for deg?
-----------------------------------------

Se for deg at du kan få en digital sparringspartner som alltid har tid og som raskt kan analysere, oppsummere, oversette, forenkle eller hente fram relevant informasjon for deg. 
Kunstig intelligens kan være denne digitale sparringspartneren.

Den jobber raskt og kan behandle store mengder data, men den har hverken din fagekspertise eller erfaring fra UiO. Den har heller ikke emosjoner, etisk eller moralsk dømmekraft, faglig skjønn eller bevissthet rundt IT-sikkerhet. 
Derfor skal du alltid bruke din kompetanse til å vurdere hvilke oppgaver som er trygge bruke KI til, vurdere KI-svar og ta det endelige ansvaret for beslutninger. 
Du må også huske at som ansatt i staten forvalter du ressurser på vegne av samfunnet. Da må du være bevisst dine valg rundt ny teknologi og ta den i bruk på en måte som ivaretar tilliten mellom forvaltningen og samfunnet. 

Vi håper dette kurset gir deg kompetanse til å bruke kunstig intelligens på en god og ansvarlig måte i jobben din.


Gjennomføring
----------------

Kurset inneholder både teori, refleksjonsoppgaver, bonusinnhold og øvingsoppgaver. 

- Teori: Kursets teori er ment å gi et godt grunnlag for å forstå de viktigste elementene ved KI-teknologien, dens styrker, svakheter og hvordan bruke KI på en god måte.
- Refleksjonsoppgaver og øvinger: Alle kapitlene inneholder oppfordringer til å øve og reflektere. Sett av tid til disse!
- Bonusinnhold: Gjennom kurset finner du muligheter til å gå litt i dybden på temaer.
- Praktiske øvingsoppgaver: På slutten av kurset finner du øvingsoppgaver for å trene på KI og utforske mer om hvordan du kan bruke det i egen arbeidshverdag. Disse kan lastes ned som en pdf. 

Kurset kan tas samlet i én omgang eller deles opp i to deler.
Hvis du vil ta kurset i to deler, anbefaler vi at du tar kapitlene 1-3 som del 1 og kapitlene 4-6 som del 2.

Hele kurset tar ca 60 minutter å gjennomføre, utenom øvingsoppgavene i siste del. 


Kursets kapitler: 
++++++++++++++++++

- :doc:`Kapittel 1: Introduksjon til «KI-språket» <module1/introduksjon-til-ki-spraket>` (8 min)
- :doc:`Kapittel 2: Store språkmodeller <module2/store-sprakmodeller>` (10 min)
- :doc:`Kapittel 3: Instruksjoner: Bli god på KI <module3/hvordan-bruke-ki-pa-en-god-mate>` (13 min)
- :doc:`Kapittel 4: Ansvarlig og bærekraftig bruk av KI <module4/ansvarlig-og-baerekraftig-bruk-av-ki>` (8 min)
- :doc:`Kapittel 5: Godkjente KI-tjenester på UiO <module5/ki-tjenester-ved-uio>` (14 min)
- :doc:`Kapittel 6: Avslutning og oppgavesett <module6/avslutning>` (6 min)

Estimert tidsbruk er basert på antall ord, og tar ikke hensyn til lengre tidsbruk på refleksjons- og øvingsoppgaver.

.. raw:: html

   <p style="font-size: 2.0rem;">Godt kurs!</p>

.. raw:: html
   
   <div style="text-align: left; padding-top: 4px;">
   <a class="btn btn-large btn-primary"
   style="display: inline-flex;
          align-items: center;
          justify-content: center;
          gap: 10px;
          padding: 10px 28px;
          background: #92ca61;
          font-size: 1.5rem;
          cursor: pointer;
          font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
          border-radius: 6px;
          color: #000000 !important;
          text-decoration: none !important;
          border: medium none currentcolor;"
   href="https://uio.instructure.com/courses/63248/pages/introduksjon-til-ki-spraket-3"
   data-api-endpoint="https://uio.instructure.com/api/v1/courses/63248/pages/introduksjon-til-ki-spraket-3"
   data-api-returntype="Page">Start kurset &rarr;&nbsp;</a>


