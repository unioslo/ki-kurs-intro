...
# Add mean lines
ax1.axhline(y=len(pre_df)/5, color=color_pre, linestyle='--', alpha=0.5, linewidth=1)
ax1.text(4.5, len(pre_df)/5 + 2, f'Gjennomsnitt pre: {pre_df["q1_understanding_ai"].mean():.2f}',
         color=color_pre, fontsize=9)
ax1.text(4.5, len(pre_df)/5 - 4, f'Gjennomsnitt post: {post_df["q1_understanding_ai"].mean():.2f}',
         color=color_post, fontsize=9)
...
# Individual plot 1: Understanding improvement
...
# Individual plot 2: Perceptions comparison
fig2, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))

labels = ['Mer effektivt', 'Mangler ferdigheter', 'Personvern bekymring',
          'Mangler verktøy', 'Ikke relevant']
pre_perceptions = count_perceptions(pre_df['q4_perceptions'])
post_perceptions = count_perceptions(post_df['q4_perceptions'])
values_pre = [pre_perceptions.get(letter, 0) for letter in ['A', 'B', 'C', 'D', 'E']]
values_post = [post_perceptions.get(letter, 0) for letter in ['A', 'B', 'C', 'D', 'E']]

ax1.barh(labels, values_pre, color=color_pre, alpha=0.8)
ax1.set_xlabel('Antall respondenter', fontsize=12)
ax1.set_title('Oppfatning av AI - Pre-kurs', fontsize=12, fontweight='bold')
ax1.grid(axis='x', alpha=0.3)

ax2.barh(labels, values_post, color=color_post, alpha=0.8)
ax2.set_xlabel('Antall respondenter', fontsize=12)
ax2.set_title('Oppfatning av AI - Post-kurs', fontsize=12, fontweight='bold')
ax2.grid(axis='x', alpha=0.3)

fig2.suptitle('Endring i oppfatning av AI etter kurs', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('survey-data/q4_perceptions_comparison.png', dpi=300, bbox_inches='tight')
print("✓ Individual plot saved: survey-data/q4_perceptions_comparison.png")
plt.close()
...
print("  - survey-data/q4_perceptions_comparison.png")
